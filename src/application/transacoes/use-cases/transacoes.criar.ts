    ...
    public async executar(req: AutenticacaoPayload,dadosNovaTransacao: DadosNovaTransacao): Promise<TransacaoEntidade | null> {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (tipo === 'TRANSFERENCIA' && !documentoCarteiraDestino) {
                reject(new BadRequestException('Documento de destino é obrigatório para transferência'));
                return;
            }
            if (valor <= 0) {
            ...
            if (tipo === 'SAQUE') {
                const usuario = this.transacoesRepositorio.buscarUsuario(documentoCarteiraOrigem);
                usuario.then(async (usuarioBuscado) => {
                    if (!usuarioBuscado) {
                    ...
                    if (carteira.saldo < valor) {
                        reject(new BadRequestException(`Saldo: ${carteira.saldo} insuficiente para saque`));
                        return;
                    }
                    ...
                    resolve(this.transacoesRepositorio.novaTransacao(
                        FabricaDeTransacoes.comIdOrigemIdDestinoValorETipo(
                            carteira.id, dadosNovaTransacao.valor, dadosNovaTransacao.tipo)));
                }).catch((error) => reject(
            ...
            if (tipo === 'TRANSFERENCIA') {
                const usuarioOrigem = this.transacoesRepositorio.buscarUsuario(documentoCarteiraOrigem);
                const usuarioDestino = this.transacoesRepositorio.buscarUsuario(documentoCarteiraDestino);
                usuarioOrigem.then(async (usuarioOrigemBuscado) => {
                    ...
                    usuarioDestino.then(async (usuarioDestinoBuscado) => {
                        if (!usuarioDestinoBuscado) {
                            reject(new BadRequestException('Usuário de destino não encontrado'));
                            return;
                        }
                        const carteiraOrigem = await this.transacoesRepositorio.buscarCarteira(
                            usuarioOrigemBuscado.id);
                        const carteiraDestino = await this.transacoesRepositorio.buscarCarteira(
                        ...
                        this.transacoesRepositorio.atualizarCarteira(
                            carteiraOrigem.id, carteiraOrigem.saldo - valor);
                        this.transacoesRepositorio.atualizarCarteira(
                            carteiraDestino.id, carteiraDestino.saldo + valor);
                        resolve(this.transacoesRepositorio.novaTransacao(
                            FabricaDeTransacoes.comIdOrigemIdDestinoValorETipo(
                                carteiraOrigem.id, dadosNovaTransacao.valor,
                                dadosNovaTransacao.tipo, carteiraDestino.id)));
                    }).catch((error) => reject(
                        new InternalServerErrorException({
