...
export class AutenticarUsuario implements IAutenticarUsuario {
  constructor(
    private readonly repositorio: IRepositorioDeAutenticacao,
    private jwtService: JwtService,
  ) {}
  async autenticar(
    credenciais: CredenciaisUsuario,
  ): Promise<{ token: string } | HttpException> {
    const autenticacao: Autenticacao = FabricaDeAutenticacao.comEmailESenha(
      credenciais.email,
      credenciais.senha,
    );
    return new Promise((resolve, reject) => {
      this.repositorio
        .buscarUsuario(autenticacao.getEmail())
