...
export class CriarUsuario {
    ...
    async criarUsuario(usuario: DadosNovoUsuario): Promise<{
        ...
        const usuarioDeDominio = FabricaDeUsuarios.comTipoNomeCompletoDocumentoEmailESenha(
            usuario.tipo, usuario.nome_completo,
            usuario.documento, usuario.email, usuario.senha);
            usuarioDeDominio.setSenha = await bcrypt.hash(usuarioDeDominio.getSenha, 10);
